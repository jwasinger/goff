...
{{- range $j := .all.NbWordsIndexesFull}}
{
	...
	{{- if eq $j 0}}
		...
		m := c[0] * {{$.V4}} 
		c[2] = madd0(m, {{$.V3}}[0], c[0])
		{{- range $i := $.all.NbWordsIndexesNoZero}}
			c[1], c[0] = madd1(v, {{$.V2}}[{{$i}}], c[1])
			{{- if eq $i $.all.NbWordsLastIndex}}
				t[{{sub $.all.NbWords 1}}], t[{{sub $i 1}}]  = madd3(m, {{$.V3}}[{{$i}}], c[0], c[2], c[1])
			{{- else}}
				c[2], t[{{sub $i 1}}] = madd2(m, {{$.V3}}[{{$i}}], c[2], c[0])
			{{- end}}
		{{- end}}
	{{- else if eq $j $.all.NbWordsLastIndex}}
		...
		{{- range $i := $.all.NbWordsIndexesNoZero}}
			...
			{{- if eq $i $.all.NbWordsLastIndex}}
				z[{{sub $.all.NbWords 1}}], z[{{sub $i 1}}] = madd3(m, {{$.V3}}[{{$i}}], c[0], c[2], c[1])
			{{- else}}
				c[2], z[{{sub $i 1}}] = madd2(m, {{$.V3}}[{{$i}}],  c[2], c[0])
			{{- end}}
		{{- end}}
	{{- else}}
		c[1], c[0] = madd1(v, {{$.V2}}[0], t[0])
